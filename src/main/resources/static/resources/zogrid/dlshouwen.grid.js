...
/*!
 * 基于DLShouWen Gridv1.2.1所做的二次开发
 * 
 * 暂且起名为 ZoGrid
 * 
 * 个人博客：blog.csdn.net/chemmuxin1993
 * 
 */
...
(function($) {
	...
	$.fn.dlshouwen.grid = {
			...
			var gridObject = {
				...
				load : function(){
					...
					if(!gridReflectionObj.option.ajaxLoad||gridReflectionObj.option.loadAll){
						...
						pager.size = gridReflectionObj.pager.pageSize;
						pager.page = gridReflectionObj.pager.nowPage - 1;
						if(gridReflectionObj.sortParameter.sortType != "null"){
							//拼接排序（&sort=排序列,排序方式）
							pager.sort = gridReflectionObj.sortParameter.columnId+","+gridReflectionObj.sortParameter.sortType;
						}
						if(gridReflectionObj.fastQueryParameters != null){
							pager = $.extend(gridReflectionObj.fastQueryParameters, pager);
							//快速查询地址，没有配置的话则默认为loadURL
							if(gridReflectionObj.option.fastQueryURL != null){
								url = gridReflectionObj.option.fastQueryURL;
							}
						}
						var option_tmp = {
							data : pager,
							...
							success:function(pager){
								gridReflectionObj.hideProcessBar(function(){
									var data_tmp = pager.content;
									var pager_tmp = {
										pageSize : pager.size,
										nowPage : pager.number + 1,
										recordCount : pager.totalElements,
										pageCount : pager.totalPages
									};
									//ajax返回的pager必须包含数据和分页，默认形式为spring mvc的Page封装类：数据路径content，分页路径/
									if(gridReflectionObj.option.dataPath != null){
										//路径配置支持'.'分级，首先分隔
										var dataPaths = gridReflectionObj.option.dataPath.split(".");
										var data_tmp_for = pager;
										for(var curPath in dataPaths){
											data_tmp_for = data_tmp_for[dataPaths[curPath]];
										}
										data_tmp = data_tmp_for;
									}
									//优先配置详细分页路径
									if(gridReflectionObj.option.pagerDetailPath != null){
										var pagerPaths = gridReflectionObj.option.pagerDetailPath.pagerRootPath.split(".");
										var pager_tmp_for = pager;
										for(var curPath in pagerPaths){
											pager_tmp_for = pager_tmp_for[pagerPaths[curPath]];
										}
										pager_tmp.pageSize = pager_tmp_for.size;
										pager_tmp.nowPage = pager_tmp_for.number + (gridReflectionObj.option.pagerDetailPath.firstPage == 0?1:0);
										pager_tmp.recordCount = pager_tmp_for.totalElements;
										pager_tmp.pageCount = pager_tmp_for.totalPages;
									}else if(gridReflectionObj.option.pagerPath != null){
										var pagerPaths = gridReflectionObj.option.pagerPath.split(".");
										var pager_tmp_for = pager;
										for(var curPath in pagerPaths){
											pager_tmp_for = pager_tmp_for[pagerPaths[curPath]];
											pager_tmp.pageSize = pager_tmp_for.size;
											pager_tmp.nowPage = pager_tmp_for.number + 1;
											pager_tmp.recordCount = pager_tmp_for.totalElements;
											pager_tmp.pageCount = pager_tmp_for.totalPages;
										}
									}
									//处理展示数据和分页相关信息
									gridReflectionObj.exhibitDatas = data_tmp;
									//获取排序数据集备份
									gridReflectionObj.sortOriginalDatas = gridReflectionObj.exhibitDatas.slice(0, gridReflectionObj.exhibitDatas.length);
									//处理分页信息
									gridReflectionObj.pager = pager_tmp;
									//构建表格、工具条
									gridReflectionObj.constructGrid();
									gridReflectionObj.constructGridPageBar();
								});
						...
						//如果为配置型增删改查需要加上操作标志为读取
						if(gridReflectionObj.option.curdURL != null){
							option_tmp.op = "retrieve";
						}
						$.fn.dlshouwen.grid.tools.curd(option_tmp,gridReflectionObj);
				...
				constructGrid : function(){
					...
					if(gridReflectionObj.option.showHeader!=false){
						...
						for(var i=0; i<columns.length; i++){
							...
							if(columns[i].type == "controll"){
								gridContent += '		'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].columnControll.title;
							}else{
					...
					if(gridReflectionObj.exhibitDatas!=null){
						for(var i=0; i<gridReflectionObj.exhibitDatas.length; i++){
							...
							for(var j=0; j<columns.length; j++){
								...
								if(columns[j].loadConfig){
									if(columns[j].loadConfig.resourcePath){
										//解析急加载资源
										value = gridReflectionObj.exhibitDatas[i][columns[j].loadConfig.resourcePath]?gridReflectionObj.exhibitDatas[i][columns[j].loadConfig.resourcePath][columns[j].loadConfig.show]:null;
									}else if(columns[j].loadConfig.resourceFrom){
										//解析懒加载资源
										var tmp_paths = columns[j].loadConfig.resourceFrom.split(".");
										var tmp_resource_url = gridReflectionObj.exhibitDatas[i];
										for(var tmp_index in tmp_paths){
											tmp_resource_url = tmp_resource_url[tmp_paths[tmp_index]];
										}
										//这一步拿到懒加载资源url,进行ajax同步请求
										$.ajax({
											url:tmp_resource_url,
											async:false,
											success:function(lazy_resource){
												value = lazy_resource[columns[j].loadConfig.show];
												//懒加载时缓存id到数据集，用于编辑时填充，不需要第二次加载
												if(!gridReflectionObj.exhibitDatas[i][columns[j].id]){
													gridReflectionObj.exhibitDatas[i][columns[j].id] = {};
												}
												var ids = columns[j].loadConfig.id.split(".");
												var tmp_href = lazy_resource;
												for(var tmp_id_index in ids){
													tmp_href = tmp_href[ids[tmp_id_index]];
												}
												gridReflectionObj.exhibitDatas[i][columns[j].id] = tmp_href;
											},
											error:function(){
												value = null;
											}
								...
								if(columns[j].type == "controll"){
									if(columns[j].edit_btn){
										var edit = '<button class="btn btn-xs btn-default dlshouwen-grid-edit" columnNo='+ j +' dataNo='+ i +'>'+ $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].buttons.edit +'</button>&nbsp;';
										gridContent += edit;
									}
									if(columns[j].del_btn){
										var del = '<button class="btn btn-xs btn-danger dlshouwen-grid-del" columnNo='+ j +' dataNo='+ i +'>' + $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].buttons.del + '</button>&nbsp;';
										gridContent += del;
										
									}
									//存在自定义按钮
									if(columns[j].custom_btn){
										gridReflectionObj.customButton = new Array();
										for(var $i = 0; $i < columns[j].custom_btn.length; $i++){
											var cur_btn = columns[j].custom_btn[$i];
											cur_btn = $.extend({}, $.fn.dlshouwen.grid.defaultOptions.defaultButton, cur_btn);
											gridContent += '<button class="'+ cur_btn.style_class +' dlshouwen-grid-btn-'+ $i +'" btnNo="'+ $i +'" columnNo='+ j +' dataNo='+ i +'><i class="'+ cur_btn.icon +'"></i>&nbsp;&nbsp;' + cur_btn.text + '</button>&nbsp;';
											//自定义按钮存在标志
											gridReflectionObj.customButton.push({
												container : ".dlshouwen-grid-btn-" + $i,
												callback : cur_btn.callback
											});
										}
									}
									gridContent += '	</td>';
					...
					//如果存在自定义按钮则绑定相对应事件
					if(gridReflectionObj.customButton){
						for(var $cur in gridReflectionObj.customButton){
							$("#dlshouwen_grid_"+gridId).on("click",gridReflectionObj.customButton[$cur].container,function(e){
								gridReflectionObj.customButton[$(this).attr("btnNo")].callback(gridReflectionObj.exhibitDatas[$(this).attr("dataNo")], $(this).attr("dataNo"));
							});
						}
					}
					//绑定编辑事件
					$("#dlshouwen_grid_"+gridId).on("click",".dlshouwen-grid-edit",function(e){
						var editFunction = gridReflectionObj.option.columns[$(this).attr("columnNo")].edit_function;
						var record = gridReflectionObj.exhibitDatas[$(this).attr("dataNo")];
						if(editFunction){
							editFunction(record);
						}else{
							//默认编辑方法
							gridReflectionObj.newData(true,record);
						}
					});
					//绑定删除事件
					$("#dlshouwen_grid_"+gridId).on("click",".dlshouwen-grid-del",function(e){
						var delFunction = gridReflectionObj.option.columns[$(this).attr("columnNo")].del_function;
						var record = gridReflectionObj.exhibitDatas[$(this).attr("dataNo")];
						if(delFunction){
							//回调定制的删除事件，接收id,record整行记录,当前行所在总数据行里的编号
							delFunction(record.id, record, $(this).attr("dataNo"));
						}else{
							var default_function = function() { 
					        	var option_tmp = {
										type:"delete",
										url:gridReflectionObj.option.loadURL+"/"+record.id,
										success:function(){
											if(window.swal){
												swal($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.successTitle,$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.successText,"success");
											}else{
												alert($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.successText);
											}
					        	...
					        	if(gridReflectionObj.option.curdURL != null){
					        		option_tmp.op = "delete";
					        		option_tmp.id = record.id;
					        		//传输一个id的属性，因为删除通常有两种手段获取删除的资源id，一个是路径匹配，一个是参数匹配，把两个都传输过去有备无患
					        		option_tmp.data = {id:record.id};
					        	}
								$.fn.dlshouwen.grid.tools.curd(option_tmp,gridReflectionObj);
					        }
							if(window.swal){
								//默认的删除操作
								swal({
						            title: $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.confirmTitle,  
						            text: $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.confirmText,  
						            type: "warning", 
						            showCancelButton: true, 
						            closeOnConfirm: false, 
						            confirmButtonText: $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.confirmButtonText, 
						            confirmButtonColor: "#ec6c62",
						            cancelButtonText: $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.cancelButtonText
						        }, default_function);
							}else{
								if(confirm($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.confirmText)){
									default_function();
								}
							}
						}
					...
					$('#dlshouwen_grid_'+gridId+' .can-sort').click(function(){
						...
						if(!gridReflectionObj.sortParameter.columnId||gridReflectionObj.sortParameter.columnId!=columnId){
							...
							gridReflectionObj.sortParameter.sortType = "asc";
						}else{
							var sortType = gridReflectionObj.sortParameter.sortType;
							if(sortType=="null"){
								gridReflectionObj.sortParameter.sortType = "asc";
							}else if(sortType=="asc"){
								gridReflectionObj.sortParameter.sortType = "desc";
							}else if(sortType=="desc"){
								gridReflectionObj.sortParameter.sortType = "null";
							}
						}
						//重新加载数据
						gridReflectionObj.reload(true);
				...
				toolbar : function(type){
					...
					var toolbars = {
						...
						newData : '<li title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.newDataTitle+'"><a href="javascript:void(0);" id="newData_replaceId">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.newData+'</a></li>',
						importExcel : '<li title="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.importExcelTitle+'"><a href="javascript:void(0);" id="importExcel_replaceId">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].toolbar.importExcel+'</a></li>',
				...
				constructGridToolBar : function(){
					...
					var customTools = gridReflectionObj.option.customTools;
					for(var i=0; i<tools.split('|').length; i++){
						...
						if(tool=='refresh'){
							...
							$('#refreshGrid_'+gridReflectionObj.option.id).click(function(){
								gridReflectionObj.fastQueryParameters = null;
								gridReflectionObj.pager.nowPage = 1;
								gridReflectionObj.reload(true);
							});
						}
						...
						//处理新增
						if(tool=='newData'){
							var content = $.fn.dlshouwen.grid.tools.replaceAll(gridReflectionObj.toolbar('newData'), 'replaceId', gridReflectionObj.option.id);
							$('#'+gridReflectionObj.option.toolbarContainer+' .dlshouwen-grid-tools').append(content);
							//绑定方法
							$('#newData_'+gridReflectionObj.option.id).click(function(){
								gridReflectionObj.newData();
							});
						}
						//处理导入
						if(tool=='importExcel'){
							var content = $.fn.dlshouwen.grid.tools.replaceAll(gridReflectionObj.toolbar('importExcel'), 'replaceId', gridReflectionObj.option.id);
							$('#'+gridReflectionObj.option.toolbarContainer+' .dlshouwen-grid-tools').append(content);
							//绑定方法
							$('#importExcel_'+gridReflectionObj.option.id).click(function(){
								gridReflectionObj.importExcel();
							});
						}
						//处理导出
					...
					if(customTools != null){
						for(var $i = 0; $i < customTools.length; $i++){
							var curTool = customTools[$i];
							var content = '<li title="'+ curTool.title +'"><a href="javascript:void(0);" toolNo="'+ $i +'" id="refreshGrid_tool_custom_'+ $i +'"><i class="'+ curTool.icon +'"></i></a></li>'; 
							$('#'+gridReflectionObj.option.toolbarContainer+' .dlshouwen-grid-tools').append(content);
							//绑定方法,回调并传入当前表格展示的源数据
							$('#refreshGrid_tool_custom_'+$i).click(function(){
								gridReflectionObj.option.customTools[$(this).attr("toolNo")].callback(gridReflectionObj.exhibitDatas);
							});
				 ...
				 * 新增方法
				 */
				newData : function(isEdit,record){
					//定义表格对象映像
					var gridReflectionObj = this;
					//如果已经初始化，则调用显示
					if(gridReflectionObj.init.newDataWindowIsInit){
						/**
						 * 编辑时填充回数据，非编辑则重置为空值
						 */
						var elements = document.forms['newDataForm_'+gridReflectionObj.option.id].elements;
						for(var i = 0; i < elements.length; i++){
							var element = elements[i];
							if(isEdit){
								if(element.getAttribute("equalWith") != ""){
									element.value = record[element.getAttribute("equalWith")];
								}else{
									element.value = record[element.getAttribute("name")] instanceof Object?record[element.getAttribute("name")][element.getAttribute("loadConfig")]:record[element.getAttribute("name")];
								}
							}else{
								element.value = "";
							}
						...
						if(isEdit){
							$('#doNewData_'+gridReflectionObj.option.id).html($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].newData.buttons.edit);
							$('#doNewData_'+gridReflectionObj.option.id).attr("action","edit");
							$('#doNewData_'+gridReflectionObj.option.id).attr("edit_id",record.id);
						}else{
							$('#doNewData_'+gridReflectionObj.option.id).html($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].newData.buttons.save);
							$('#doNewData_'+gridReflectionObj.option.id).attr("action","save");
						}
						$('#new_data_'+gridReflectionObj.option.id+'_modal').modal('show');
						return;
					}
					var content = '';
					content += $.fn.dlshouwen.grid.tools.getWindowStart('new_data_'+gridReflectionObj.option.id, $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].newData.title);
					content += '<form id="newDataForm_'+gridReflectionObj.option.id+'">';
					...
					for(var j=0; j<gridReflectionObj.option.columns.length; j++){
						...
						if(column.newData==true){
							content += '<div class="form-group">';
							content += '	<label class="col-sm-4 control-label text-right">'+column.title+'：</label>';
							//本地数据列表解析
							if(column.codeTable){
								content += '<div class="col-sm-5">';
								content += '	<select class="form-control" id="'+column.id+'" name="'+column.id+'" equalWith="'+ (column.equalWith?column.equalWith:"") +'" validate="'+ (column.validate != undefined?j:"") +'">';
								...
								content += '	</select>';
								content += '</div>';
							}else if(column.loadConfig){
								var resourceSetUrl = column.loadConfig.resourceSetUrl;
								var tmp_resource_set = resourceSetUrl.split(">");
								$.ajax({
									url:tmp_resource_set[0],
									async:false,
									success:function(return_resource_set){
										for(var tmp_index = 1; tmp_index < tmp_resource_set.length; tmp_index++){
											return_resource_set = return_resource_set[tmp_resource_set[tmp_index]];
										}
										//配置型非直接加载解析
										content += '<div class="col-sm-5">';
										content += '	<select class="form-control" id="'+column.id+'" name="'+column.id+'" loadConfig="'+ column.loadConfig.id +'" resourcePath="'+column.loadConfig.resourcePath+'" equalWith="'+ (column.equalWith?column.equalWith:"") +'" validate="'+ (column.validate != undefined?j:"") +'">';
										content += '		<option value="">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.codeTableSelectAll+'</option>';
										for(var key in return_resource_set){
											var ids = column.loadConfig.id.split(".");
											var href = return_resource_set[key];
											for(var tmp_id_index in ids){
												href = href[ids[tmp_id_index]];
											}
											content += '	<option value="'+href+'">'+return_resource_set[key][column.loadConfig.show]+'</option>';
										}
										content += '	</select>';
										content += '</div>';
									}
								...
								if(column.type=='date'){
									...
									content += '	<input type="text" class="form-control" id="'+column.id+'" equalWith="'+ (column.equalWith?column.equalWith:"") +'" name="'+column.id+'" dataType="date" validate="'+ (column.validate != undefined?j:"") +'" format="'+column.format+'" placeholder="'+hoderName+column.title+'" onclick="WdatePicker({dateFmt:\''+$.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H')+'\'});" />';
									content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
									content += '</div>';
								}else{
									content += '<input type="'+ (column.type=="password"?("password"):("text")) +'" equalWith="'+ (column.equalWith?column.equalWith:"") +'" class="form-control" id="'+column.id+'" name="'+column.id+'" validate="'+ (column.validate != undefined?j:"") +'" placeholder="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.input+column.title+'" />';
								}
								content += '</div>';
							}
							content += '</div>';
					...
					content += '</form>';
					var buttons = '';
					buttons += '<button type="button" class="btn btn-warning" id="resetNewData_'+gridReflectionObj.option.id+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].newData.buttons.reset+'</button>';
					buttons += '<button type="button" class="btn btn-primary" action="new" id="doNewData_'+gridReflectionObj.option.id+'">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].newData.buttons.save+'</button>';
					content += $.fn.dlshouwen.grid.tools.getWindowEnd($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].buttons.close, buttons);
					$('body').append(content);
					//编辑时填充回数据，如果非编辑则重置为空值
					var elements = document.forms['newDataForm_'+gridReflectionObj.option.id].elements;
					for(var i = 0; i < elements.length; i++){
						var element = elements[i];
						if(isEdit){
					...
					if(isEdit){
						...
						$('#doNewData_'+gridReflectionObj.option.id).html($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].newData.buttons.save);
						$('#doNewData_'+gridReflectionObj.option.id).attr("action","new");
					}
					
					//绑定方法
					$('#resetNewData_'+gridReflectionObj.option.id).click(function(){
						document.forms['newDataForm_'+gridReflectionObj.option.id].reset();
					});
					$('#doNewData_'+gridReflectionObj.option.id).click(function(){
						if($(this).attr("action") == "edit"){
							gridReflectionObj.doNewData(document.forms['newDataForm_'+gridReflectionObj.option.id], true, $(this).attr("edit_id"));
						}else{
							gridReflectionObj.doNewData(document.forms['newDataForm_'+gridReflectionObj.option.id]);
						}
					...
					$('#new_data_'+gridReflectionObj.option.id+'_modal').modal('show');
					//标识初始化完成
					gridReflectionObj.init.newDataWindowIsInit = true;
				},
				//新增方法
				doNewData : function(form, isEdit, id){
					//定义表格对象映像
					var gridReflectionObj = this;
					var data = new Object();
					//遍历获取新增参数
					...
					for(var i=0;i<elements.length;i++){
						var element = elements[i];
						var validate_column = gridReflectionObj.option.columns[element.getAttribute("validate")];
						//验证
						if(validate_column && validate_column.validate && validate_column.validate != ""){
							if(!$.fn.dlshouwen.grid.validate(validate_column.validate,element.value,element,(validate_column.validateText?validate_column.validateText:null),$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang])){
								return;
							}
						}
						if(element.getAttribute("equalWith")){
							var target = $('#new_data_'+gridReflectionObj.option.id+'_modal').find("#"+element.getAttribute("equalWith"));
							if(target.val() != element.value){
								if(window.swal){
									swal($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.warningTitle,$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].validate.equal,"warning");
								}else{
									alert($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].validate.equal);
								}
								element.style.borderColor = "#f00";
								target.css("borderColor","#f00");
								return;
							}else{
								element.style.borderColor = "#ccc";
								target.css("borderColor","#ccc");
							}
						}
						if($.fn.dlshouwen.grid.tools.equalsIgnoreCase(element.type, 'text') 
							...
							if (element.name != "" && element.value != ""){
								if(element.getAttribute("dataType") != undefined){
									data[element.name] = new Date(element.value).getTime();
								}else if(element.getAttribute("loadConfig") && element.getAttribute("resourcePath") == undefined){
									data[element.name] = {};
									data[element.name][element.getAttribute("loadConfig")] = element.value;
								}else{
					...
					if(gridReflectionObj.option.curdURL != null){
						if(isEdit){
							option_tmp.op = "update";
						}else{
							option_tmp.op = "create";
						}
					}else if(isEdit){
						option_tmp.type = "patch";
						option_tmp.url = gridReflectionObj.option.loadURL + "/" + id;
					}else{
						option_tmp.type = "post";
					}
					$.fn.dlshouwen.grid.tools.curd(option_tmp, gridReflectionObj);
					$('#new_data_'+gridReflectionObj.option.id+'_modal').modal('hide');
					form.reset();
				},
				/**
				 * 导入excel
				...
				importExcel : function(){
					//定义表格对象映像
					var gridReflectionObj = this;
					var form_id = "form_" + gridReflectionObj.option.id;
					if($("#" + form_id).length == 0){
						var form = '<form enctype="multipart/form-data" id="'+ form_id +'">'+
						'  <input type="file" name="files" accept=".xls,.xlsx" id="import_files" multiple="multiple" />'+
						'</form>';
						$("body").append(form);
						$("#import_files").change(function(){
							if($("#import_files").val() == null || $("#import_files").val() == ""){
					    		return;
					    	}
							var formData = new FormData(document.getElementById(form_id));
							formData.append("action",gridReflectionObj.option.importAction);
							  $.ajax({
							      url: gridReflectionObj.option.importURL,
							      type: 'POST',
							      data: formData,
							      cache: false,
							      contentType: false,
					...
					for(var j=0; j<gridReflectionObj.option.columns.length; j++){
						...
						if(column.fastQuery==true){
							...
							if(column.fastQueryType=='range'){
								...
								content += '<div class="col-sm-5">';
								content += '	<select class="form-control" id="'+column.id+'" name="'+column.id+'">';
								content += '		<option value="">'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.codeTableSelectAll+'</option>';
								for(var key in column.codeTable){
									content += '	<option value="'+key+'">'+column.codeTable[key]+'</option>';
								}
								...
								if(column.type=='date'){
									...
									content += '	<input type="text" class="form-control" id="'+column.id+'" name="'+column.id+'" format="'+column.format+'" placeholder="'+hoderName+column.title+'" onclick="WdatePicker({dateFmt:\''+$.fn.dlshouwen.grid.tools.replaceAll(column.format, 'h', 'H')+'\'});" />';
									content += '	<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
									content += '</div>';
								}else{
									content += '<input type="text" class="form-control" id="'+column.id+'" name="'+column.id+'" placeholder="'+$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].fastQuery.input+column.title+'" />';
								}
								content += '</div>';
							}
							content += '</div>';
				...
				print : function(){
					...
					for(var i=0; i<gridReflectionObj.option.columns.length; i++){
						...
						if(column.print==false || column.type == "controll"){
							continue;
						}
						//获取记录号
				...
				exportFile : function(exportType){
					...
					for(var i=0; i<gridReflectionObj.option.columns.length; i++){
						var column = gridReflectionObj.option.columns[i];
						if(column['export']==false || column.type == "controll"){
							continue;
						}
					...
					$('#dlshouwen_grid_export_execute_'+exportType+'_'+gridReflectionObj.option.id).click(function(){
						...
						gridPager.size = gridReflectionObj.pager.pageSize;
						gridPager.page = gridReflectionObj.pager.nowPage - 1;
						...
						$('input[id*=dlshouwen_grid_export_check_'+exportType+'_'+gridReflectionObj.option.id+'_]:checked').each(function(){
							exportColumns.push(gridReflectionObj.option.columns[this.value].id);
						});
						gridPager.exportColumns = exportColumns;
						$.fn.dlshouwen.grid.tools.ajax({
							type : "post",
							url : gridReflectionObj.option.exportURL,
							data : JSON.stringify(gridPager)
						},gridReflectionObj);
		...
		tools : {
			...
			/**
			 * 增删改查统一调用入口，rest需要指定option.type，配置型增删改查需要指定option.op(操作类型)[,option.id(路径匹配)]
			 * 
			 */
			curd: function(option,gridReflectionObj){
				if(gridReflectionObj.option.curdURL != null){
					//需要把ajax提交方式修正：查删为get、增改为post
					option.type = "get";
					if(option.op == "create"){
						//增加
						option.url = gridReflectionObj.option.curdURL.create;
						option.type = "post"
					}else if(option.op == "retrieve"){
						if(gridReflectionObj.fastQueryParameters != null){
							option.url = gridReflectionObj.option.fastQueryURL;
						}else{
							option.url = gridReflectionObj.option.curdURL.retrieve;
						}
					}else if(option.op == "update"){
						option.url = gridReflectionObj.option.curdURL.update;
						option.type = "post";
					}else if(option.op == "patch"){
						option.url = gridReflectionObj.option.curdURL.patch;
						option.type = "post";
					}else if(option.op == "delete"){
						option.url = gridReflectionObj.option.curdURL.delete;
					}
					//存在路径匹配
					if(option.url.indexOf("{id}") != -1){
						option.url = option.url.replace("{id}",option.id);
					}
				}
				this.ajax(option,gridReflectionObj);
			},
			//rest型增删改查调用
			ajax: function(option,gridReflectionObj){
				var defaultOption = {
						type : "get",
						url : gridReflectionObj.option.loadURL,
						async : true,
						data : null,
						success : function(){
							if(window.swal){
						...
						error : function(XMLHttpRequest, textStatus, errorThrown){
							...
							if(window.swal){
								swal($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.failTitle,$.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.failText,"warning");
							}else{
								alert($.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].alert.failText);
							}
						}
				}
...
(function($){
	$.fn.dlshouwen.grid.validate = function(type,value,focus,custom_alert,language){
			/**
			 * 统一正则验证入口函数
			 * 支持自定义正则，支持扩展任何复杂验证
			 * 
			 ...
			 * 	//使用已有"name"规则验证，值为name容器的value
			 * 	validate("name",name.value);
			 * 	//使用已有"name"规则验证，值为name容器的value，验证不通过name容器获取焦点，验证不通过提示语为"请输入正确的姓名"
			 * 	validate("name",name.value,name,"请输入正确的姓名");
			 * 	//使用自定义规则验证，规则名为twoNum，传入自定义规则
			 * 	var num = document.getElementById("num");
			 * 	validate(/^\d{2}$/,num.value,num,"请输入正确的姓名");
			 *  
			 *	//扩充复杂验证：
			 * 	preValidate.identity = IdentityCodeValid;
			 * 	其中，IdentityCodeValid为一个Function函数，函数可定义任何验证类型，遵守验证通过返回true，反之返回false即可
			 * author : zo
			 * 
			 * @param {String|RegExp} type 	正则验证类型				(参数必选，否则永远匹配失败)
			 * @param {String} value 	要验证的值					(参数必选，否则永远匹配失败)
			 * @param {DOM}    focus 	验证失败时自动获取焦点的容器		(参数可选)
			 * @param {String} alert 	验证失败时提示文本			(参数可选)
			 */
			var isRight = false;
			var alertText = language.alert.requiredText;
			var preValidate = new Object();
			preValidate.code = new RegExp(/^\d{4,8}$/);
			preValidate.tel = new RegExp(/^1[34578]\d{9}$/);
			preValidate.name = new RegExp(/^[\u4e00-\u9fa5]{2,4}$/);
			preValidate.identity = IdentityCodeValid;
			preValidate.password = new RegExp(/^\w{6,16}$/);
			preValidate.email = new RegExp(/^\w +[@]\w +[.][\w.] +$/);
			
			var errorAlert = new Object();
			errorAlert.code = language.validate.code;
			errorAlert.tel = language.validate.tel;
			errorAlert.name = language.validate.name;
			errorAlert.identity = language.validate.identity;
			errorAlert.password = language.validate.password;
			errorAlert.email = language.validate.email;
			errorAlert.required = language.alert.requiredText;
			
			if(custom_alert){
				alertText = custom_alert;
			}else{
				alertText = errorAlert[type];
			}
			while(true){
				
				if(type == undefined || !type instanceof String || !type instanceof RegExp){
					break;
				}
				if(type instanceof RegExp && !type.test(value)){
					if(window.swal){
						swal(language.alert.warningTitle,(alertText?alertText:language.validate.defaultAlert),"warning");
					}else{
						alert((alertText?alertText:language.validate.defaultAlert));
					}
					if(focus){
						focus.style.borderColor="#f00";
					}
					break;
				}
				//验证值应当不为空
				if(value == undefined || value == "" || value == null){
					if(window.swal){
						swal(language.alert.warningTitle,alertText,"warning");
					}else{
						alert((alertText?alertText:language.validate.defaultAlert));
					}
					...
					focus.style.borderColor="#ccc";
					if(type == "required") return true;
				}
				
				if((preValidate[type] instanceof RegExp && !preValidate[type].test(value)) || (preValidate[type] instanceof Function && !preValidate[type](value))){
					if(swal){
						swal(language.alert.warningTitle,alertText,"warning");
					}else{
						alert((alertText?alertText:language.validate.defaultAlert));
...
(function($) {
	$.fn.dlshouwen.grid.defaultOptions = {
		...
		defaultButton : {
			style_class : "btn btn-xs btn-default",
			icon : "fa fa-check",
			text : "default",
			callback : function(){}
		}
